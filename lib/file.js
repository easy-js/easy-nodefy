...
var file = {
  ...
  getContents: function (inputPath, options, callback) {
    ...
    var parseContents = async.apply(file.parseContents, inputPath, options) ;
   ...
   * @param {function} callback - Function to execute after parsed.
   */
  parseContents: function (inputPath, options, contents, callback) {
    var err = null;
    
    // Add file directory to parse in order to resolve
    // relative paths.
    var parseOptions = _.extend({
      relDir: path.dirname(inputPath)
    }, options);

    try {
      contents = parser.parse(contents.toString(), parseOptions);
    } catch(e) {
      err = e;
    }
