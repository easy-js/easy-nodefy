...
var parser = {
  ...
  convert: function (raw, defineNode, options) {
    ...
    var requireNodes   = parser.getRequireNodes(factoryNode);
    ...
    return options && (options.map || options.paths)
      ? parser.replaceRequireNodes(output, requireNodes, options)
      : output;
  },

  /**
   * Takes a raw string and replaces any updates raw require statements
   * to use resolved paths.
   *
   * @private
   *
   * @param {string} raw - Raw file contents.
   * @param {object} factoryNode - Esprima created ast nodes of require statements
   * @param {object} options - Parse options.
   */
  replaceRequireNodes: function (raw, requireNodes, options) {
    var requires = [];

    _.each(requireNodes, function (node) {
      var dependency = node.declarations[0].init.arguments[0].value;
      var resolved = parser.resolveDependency(dependency, options);
      var original = new RegExp('require\\([\'\"]' + dependency + '[\'\"]\\)');
      var updated = 'require(\'' + resolved + '\')';
  ...
  /**
   * Determine weather a given node is a require statement.
   *
   * @private
   *
   * @param {object} node - Esprima created ast node.
  ...
  /**
   * Get any require statements declared within define factory.
   *
   * @private
   *
   * @param {object} factoryNode - Esprima created ast node of define
   *   factory.
   */
  getRequireNodes: function (factoryNode) {
    return factoryNode.body.body.filter(this.isRequire);
  },
